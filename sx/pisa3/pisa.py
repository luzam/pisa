...
class pisaLinkLoader:
    ...
    def getFileName(self, name, relative=None):
        try:
            url = six.moves.urllib.parse.urljoin(relative or self.src, name)
            ...
            ufile = six.moves.urllib.request.urlopen(url)                     
            tfile = open(path, "wb")
            while True:
    ...
    for src in a_src:
        ...
        if src=="-" or base_dir!=None:
            ...
            if src.startswith("http:") or src.startswith("https:"):                
                ...
                src = "".join(six.moves.urllib.parse.urlsplit(src)[1:3]).replace("/", "-")                                
            else:
